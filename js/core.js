...
;(function($){
	window.FJSCore={
		...
		,defState:''
		,defMobileState:'aboutPage.html'
		,defStateMobileText:'01. About us'
...
if(!FJSCore.mobile)
	$(function longScroller(){
		...
		,doc=$(document)
		,win=$(window)
		,scrollProp = scrollHolder.attr('data-scroll-property') ? scrollHolder.attr('data-scroll-property') : 'scrollTop'  // scrollLeft, scrollTop
		,offsetProp = (scrollProp == 'scrollLeft') ? 'offsetLeft' : 'offsetTop'
		,currentLocation
		,n=0,tmr,
		currScrollValue = 0;
	...
	if(items.length!==0){
		...
		function onresize(){
			...
			if(scrollHolder.data('switcher').curr&&scrollHolder.data('switcher').curr.length!==0) {
				$scrollable
					.stop()
					.animate({
						scrollProp:scrollHolder.data('switcher').curr.prop(offsetProp)
					},{
		...
		win
			.on('scroll',function(){
				...
				var i=0,
				func = scrollProp == 'scrollTop' ? 'height' : 'width';				
				while(map[i++]-win[func]()/2<=win[scrollProp]()){}

				if ((Core.state!==undefined&&names[i-2]!==Core.state) && (!Core.state.match(new RegExp(/(.html|.php)/g)))) {
					doc.trigger('changeLocation',currentLocation=names[i-2])
				}
			})
			.on('scrollEnable', function(e, d){
				if (d === 'true') {
					longScroller.blockScrollCalc = false;
				} else if (d === 'false') {
					longScroller.blockScrollCalc = true;
				}
			})
			.on('mousewheel',function(e, delta){
				$scrollable.stop();
				if (scrollProp === 'scrollLeft') {
					currScrollValue = (currScrollValue >= 0) ? (currScrollValue + -delta * 50) : 0;
					$scrollable.each(function(){
						$(this).stop().animate({'scrollLeft': currScrollValue},0);
					});
  					e.preventDefault();
				}
			})
		...
		items
			.on('show',function(e,d){
				if(Core.state!==currentLocation) { 
					var props = {};
					currScrollValue = d.curr.prop(offsetProp)
					props[scrollProp] = currScrollValue;
